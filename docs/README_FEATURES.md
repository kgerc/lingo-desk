...
**Backend:** `notification.routes.ts`, `notification.service.ts`, `payment-reminder.service.ts`
...
### Pełna mapa powiadomień

#### Lekcje — natychmiast po zdarzeniu

| Zdarzenie | Odbiorca | Kanał |
|-----------|----------|-------|
| Lekcja utworzona | Uczeń | IN_APP + Alert (INFO) |
| Lekcja anulowana | Uczeń + Nauczyciel | EMAIL + Alert (HIGH) |
| Lekcja przełożona (zmiana terminu) | Uczeń + Nauczyciel | EMAIL + Alert |
| Nagranie lekcji dodane | Uczeń | EMAIL (opcjonalne, flaga `sendEmail`) |

#### Przypomnienia o lekcjach — CRON `*/5 * * * *`

| Kiedy | Odbiorca | Kanał |
|-------|----------|-------|
| 24h przed lekcją | Uczeń + Nauczyciel | EMAIL + IN_APP |
| 1h przed lekcją | Uczeń + Nauczyciel | EMAIL + IN_APP |

> Respektują preferencje użytkownika (`emailReminders`, `inAppNotifications`). Zabezpieczenie anty-spam: brak duplikatu w ciągu 2 godzin.

#### Płatności — natychmiast + CRON

| Zdarzenie | Odbiorca | Kanał |
|-----------|----------|-------|
| Płatność oznaczona jako zapłacona | Uczeń | EMAIL (potwierdzenie) |
| Manager ręcznie wysyła przypomnienie | Uczeń | EMAIL (min. 24h przerwa) |

**Automatyczne przypomnienia** — cron codziennie o **9:00** (`0 9 * * *`):
- `paymentReminderDaysBefore` dni przed terminem — email z żółtym nagłówkiem ⏰
- W dniu terminu — email z pomarańczowym nagłówkiem 📅
- `paymentReminderDaysAfter` dni po terminie — email z czerwonym nagłówkiem ⚠️

Liczba dni konfigurowalna per organizacja (domyślnie: **7/3/1** przed, **1/3/7** po).

#### Budżet i zapisy

| Zdarzenie | Odbiorca | Kanał | Kiedy |
|-----------|----------|-------|-------|
| Zapis na kurs | Uczeń | EMAIL | Natychmiast |
| Saldo ≤ 2 godziny | Uczeń + (opcjonalnie Manager) | EMAIL + IN_APP | Poniedziałki 10:00 |

#### Formularze zgłoszeń (Course Applications)

| Zdarzenie | Odbiorca | Kanał |
|-----------|----------|-------|
| Złożenie zgłoszenia | Kandydat | EMAIL (potwierdzenie przyjęcia) |
| Status zmieniony na ACCEPTED/REJECTED | Kandydat | EMAIL |
| Konwersja zgłoszenia na konto ucznia | Nowy uczeń | EMAIL (dane do logowania + tymczasowe hasło) |

#### Zarządzanie użytkownikami

| Zdarzenie | Odbiorca | Kanał |
|-----------|----------|-------|
| Zaproszenie nowego użytkownika | Nowy użytkownik | EMAIL (tymczasowe hasło) |
| Reset hasła | Użytkownik | EMAIL |
| Wysłanie dokumentu | Odbiorca | EMAIL (z załącznikiem) |

#### Alerty systemowe IN_APP — generowane przez `generateSystemAlerts()`

| Alert | Priorytet | Warunek |
|-------|-----------|---------|
| Lekcje odwołane po terminie (wymagają rozliczenia) | HIGH | Lekcje z ostatnich 24h |
| Lekcje przeterminowane (nieoznaczone 24h+ po czasie) | CRITICAL | Lekcje bez statusu końcowego |
| Uczniowie bez aktywnych zapisów | NORMAL | Aktywni uczniowie bez kursu |

---

### Harmonogram CRON (podsumowanie)

| Zadanie | Cron | Strefa | Częstotliwość |
|---------|------|--------|---------------|
| Przypomnienia o lekcjach (24h i 1h przed) | `*/5 * * * *` | Europe/Warsaw | Co 5 minut |
| Alerty niskiego budżetu | `0 10 * * 1` | Europe/Warsaw | Poniedziałki 10:00 |
| Przypomnienia o płatnościach | `0 9 * * *` | Europe/Warsaw | Codziennie 9:00 |
| Czyszczenie starych powiadomień | `0 2 * * 0` | Europe/Warsaw | Niedziele 2:00 |
